...
def main(cfg: DictConfig) -> None:
    ...
    wandb_logger = WandbLogger(project="AVR_universal", log_model=False, id=None) # id=None - force new run creation
    ...
    run_config["finetuned_from"] = {"slurm_id": _id, "wandb_id": wandb_id}
    run_config = DictConfig(run_config)

    # not ideal solution, but it works
    additional_kwargs = {}
    if (increment_dataloader_idx := cfg.get("increment_dataloader_idx")) is not None:
        additional_kwargs["increment_dataloader_idx"] = increment_dataloader_idx

    # if increment_dataloader_idx is not None:
    #     old_tasks = run_config.data.tasks
    #     run_config.data.tasks = [old_tasks[increment_dataloader_idx]]
    ...
    trainer.fit(module, data_module, ckpt_path=None) # ckpt_path=None - force new (clean) training setup
