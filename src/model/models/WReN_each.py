...
class WReN_each(pl.LightningModule):
    ...
    def forward(self, context: torch.Tensor, answers: torch.Tensor) -> torch.Tensor:
        ...
        context_g_out = torch.zeros(batch_context_size, height_context * width_context * 2,
                                    device=context.device).type_as(context)
        for i in range(context.size()[1]):
            chosen_panel = context[:, i].unsqueeze(1).repeat(1, context.size()[1], 1, 1)
            pairs = self.group_objects(context, chosen_panel)

            context_g_out += self.g(pairs).sum(1)
            del pairs
            gc.collect()
        ...
        for i in range(num_answer_images):
            ...
            relations = relations / (num_context_panels ** 2)
