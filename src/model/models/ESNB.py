...
class ESNB(pl.LightningModule):
    def __init__(
        ...
        task_seg: list[list[int]] | None,  # Task segmentation (for tasksegmented_contextnorm - pass task specific information)
    ...
    def forward(self, z_seq):
        ...
        hidden = torch.zeros(1, z_seq.shape[0], self.hidden_size, device=z_seq.device)  # x_seq.shape
        cell_state = torch.zeros(1, z_seq.shape[0], self.hidden_size, device=z_seq.device)  # x_seq.shape
        # Initialize retrieved key vector
        key_r = torch.zeros(z_seq.shape[0], 1, self.key_size + 1, device=z_seq.device)  # x_seq.shape
        # Memory model
        # all_key_r = []
        # print(f"{z_seq.shape=}")
        for t in range(z_seq.shape[1]):  # x_seq.shape
            # print("*", t)
            # Image embedding
            ...
            if t == 0:
                key_r = torch.zeros(z_seq.shape[0], 1, self.key_size + 1, device=z_seq.device)  # x_seq
            else:
                # Read key
        ...
        z_t = torch.zeros(z_seq.shape[0], 1, self.z_size, device=z_seq.device)  # x_seq.shape
        # print(f"{z_t=}")
        # print(f"{lstm_out=}")
        # LSTM
        lstm_out, (hidden, cell_state) = self.lstm(key_r, (hidden, cell_state))
